...
private:
	...
	vector<int> dcInt; // dont cares in decimal
	vector<string> dcBin; // dont cares in binary
...
public:
	void initialise() {
		...
		for (int i = 0; i < nMin; ++i){
			...
			bin.push_back(util.intToBinString(nBits, x));
		}

		cout << "Enter number of don't cares:" << endl;
		cin >> nDc;

		cout << "Enter don't cares:" << endl;
		for (int i = 0; i < nDc; ++i){
			int x;
			cin >> x;
			dcInt.push_back(x);
			dcBin.push_back(util.intToBinString(nBits, x));
			bin.push_back(util.intToBinString(nBits, x));
		}
